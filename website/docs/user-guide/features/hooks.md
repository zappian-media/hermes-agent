...
Fires immediately **before every** `MemoryStore.load_from_disk()` that can freeze `MEMORY.md` / `USER.md` into a system prompt: once at agent construction (CLI, query mode, gateway, cron, and subagents), after plugin discovery, and again when context compression invalidates and rebuilds the prompt. It is the lifecycle seam before every such snapshot, not only the first one.
