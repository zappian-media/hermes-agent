...
| [`pre_memory_load`](/user-guide/features/hooks#pre_memory_load) | Before every `MemoryStore.load_from_disk()` that freezes a system-prompt memory snapshot: at agent construction and again at the context-compression rebuild boundary | `hermes_home: str, memory_dir: str, platform: str, session_id: str, projection_source: str, required: bool` | **fail-closed** directive: `{"action": "block", "message": ...}`, a raised exception, or a timeout aborts the memory load; `None` / `{"action": "allow"}` proceeds |
